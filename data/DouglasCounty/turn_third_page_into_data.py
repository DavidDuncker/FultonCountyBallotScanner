...
def read_location_data(text):
    ...
    bookmark1 = lines[1].find("PollID:")
    bookmark2 = lines[1].find("BallotID:")
    location_data["Poll ID"] = lines[1][bookmark1+7:bookmark2]
    location_data["Ballot ID"] = lines[1][bookmark2+9:]
...
def read_race_data(text, list_of_races):
    ...
    for line_index in range(0, len(lines)):
        for race in list_of_races:
            ...
            if line_sanitized == race_sanitized and len(race_sanitized) > 2:
                race_data[race] = lines[line_index+1]
